...
echo "⏳ 1/2 Correction des chemins de users.js..."
# Suppression du fichier mal placé
rm -f "$DIR/pages/api/users.js"

# Recherche du vrai fichier users et mise à jour du trial à 3 jours
REAL_USERS=$(find "$DIR/pages/api" -name "users.js" | head -n 1)
if [ -n "$REAL_USERS" ]; then
    sed -i 's/d.getDate() + 7/d.getDate() + 3/g' "$REAL_USERS"
    echo "✅ Fichier users.js corrigé : $REAL_USERS"
fi

echo "⏳ 2/2 Reconstruction complète de simulateur.js..."
...
export default function Simulateur() {
 ...
 const isPremium = ['premium_month','premium_year','premium_life','premium_lifetime','trial'].includes(plan)
