...
echo "⏳ 1/3 Réécriture propre de auth-helper.js..."
...
echo "⏳ 2/3 Réécriture propre de calcul.js (pour s'assurer qu'il utilise le profil)..."
cat << 'EOF' > "$DIR/pages/api/calcul.js"
...
export default async function handler(req, res) {
  ...
  return res.status(200).json({
    ...
    loyerCCAnn: result.loyerCCAnn,
    mensualite: result.mensualite,
    empruntCalc: result.empruntCalc,
    fonciereDed: result.fonciereDed,
    coproTotale: result.coproTotale,
    coproNonRec: result.coproNonRec,
    prixVente: result.prixVente,
    prixDispo: result.prixDispo,
    crd: result.crd,
    ira: result.ira,
    fraisAg: result.fraisAg,
    patrimoineIFI: result.patrimoineIFI,
    ifi: result.ifi,
    loan: null, years: null,
    richNetteSCI: null, richLN: null, richLMNP: null,
    triSCI: null, triLN: null, triLMNP: null,
    verdict: 'Option Premium requis'
  })
}
EOF

echo "⏳ 3/3 Réécriture propre de simulateur.js (avec le Tableau Comparatif)..."
cat << 'EOF' > "$DIR/pages/simulateur.js"
...
const Watermark = ({children}) => (
 ...
 <a href="/pricing" style={{background:T.gold,color:'#000',fontWeight:700,fontSize:13,padding:'8px 20px',borderRadius:6,textDecoration:'none',marginTop:8}}>Voir les offres →</a>
 </div>
 </div>
)
...
export default function Simulateur() {
 ...
 if(data.simulation?.params) setP(prev=>({...prev,...data.simulation.params}))
 }
 load()
 },[user])
 ...
 await fetch('/api/simulation',{method:'POST',headers:{'Content-Type':'application/json',Authorization:`Bearer ${token}`},body:JSON.stringify({params,nom:params.adresse||'Mon bien'})})
 setSaveStatus('saved')
 ...
 // Ligne simplifiée pour être sûr que l'accès débloque l'UI
 ...
 const ebeBase = (result?.loyerHCAnn || 0) - (p.gli + p.fonciere - p.teom + p.coproNonRec + p.pno + p.gestion + p.entretien);
 const compData = result ? {
   sci: { ebe: ebeBase - p.cfe - p.comptable, cashFlow: result.years?.[0]?.cfSCI, rendBrut: result.rendBrut * 100, rendNet: result.rendNet * 100, tri: result.triSCI * 100 },
   lmnp: { ebe: ebeBase - p.cfe - p.comptable, cashFlow: result.years?.[0]?.cfLMNP, rendBrut: result.rendBrut * 100, rendNet: result.rendNet * 100, tri: result.triLMNP * 100 },
   nue: { ebe: ebeBase, cashFlow: result.years?.[0]?.cfLN, rendBrut: result.rendBrut * 100, rendNet: result.rendNet * 100, tri: result.triLN * 100 }
 } : null;
 ...
 {!isPremium&&<a href="/pricing" style={{background:T.gold,color:'#000',fontWeight:700,fontSize:12,padding:'5px 12px',borderRadius:6,textDecoration:'none'}}>Premium</a>}
 ...
 {result&&<InfoBox color={T.blue}>Mensualité : <strong>{eur(result.mensualite)}/mois</strong> — Annuel : {eur(result.mensualite*12)}</InfoBox>}
 </Section>
 )}
 ...
 <div style={{height:8}}/>
 <Inp label="Charges copro récupérables locataire (€/an)" value={p.coproRec} onChange={set('coproRec')} step={100}/>
 ...
 <Inp label="Charges récupérables mensuelles (€)" value={p.charges} onChange={set('charges')} step={10}/>
 ...
 <Card label="Rendement net charges" value={pct(result.rendNet)} color={T.green} sub="Après charges, avant impôts"/>
 ...
 <TableauComparatif data={compData} />
 </Section>
 ):(
 ...
 {/* GRAPHIQUE 1 — 6 courbes CF avant + après impôt */}
 ...
 {/* GRAPHIQUE 2 — Tréso cumulée après impôt */}
 ...
 {/* HORIZONS */}
 {show('horizons')&&result&&isPremium&&(
 ...
 {/* SENSIBILITÉ */}
 {show('sensibilite')&&result&&isPremium&&(
 ...
 {show('tab_lmnp')&&result&&isPremium&&(
 ...
 {/* DÉTAIL REVENTE */}
 {show('tab_revente')&&result&&isPremium&&(
 ...
 {/* EMPRUNT */}
 {show('tab_emprunt')&&result&&isPremium&&result.loan?.length>0&&(
 ...
 <RegimeWinner name="SCI IS — net après Flat Tax 31,4%" val={result.richNetteSCI} winner={result.verdict==='SCI IS'} color={T.sci} restricted={isRestricted}/>
 <RegimeWinner name="Location Nue" val={result.richLN} winner={result.verdict==='Location Nue'} color={T.ln} restricted={isRestricted}/>
 <RegimeWinner name="LMNP Réel" val={result.richLMNP} winner={result.verdict==='LMNP Réel'} color={T.lmnp} restricted={isRestricted}/>
 </div>
 {isRestricted&&(
 <div style={{background:`${T.orange}12`,border:`1px solid ${T.orange}44`,borderRadius:8,padding:'12px 16px',fontSize:13,color:T.orange,textAlign:'center',marginBottom:12}}>
 🔒 Période d'essai terminée. <a href="/pricing" style={{color:T.gold,fontWeight:700}}>Passez Premium</a> pour accéder aux résultats complets.
 </div>
 )}
...
echo "✅ Tous les fichiers sont corrigés et sécurisés."
